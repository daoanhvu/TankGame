...
void Camera::rotate(float yawR, float pitchR, float roll) {
	...
	// N.B. 'angleAxis' method takes angle in radian
	gm::quat rotation = gm::angleAxis(pitchAccum, gm::vec3(1, 0, 0)) * gm::angleAxis(yawAccum, gm::vec3(0, 1, 0))
		* gm::angleAxis(rollAccum, gm::vec3(0, 0, 1));
	model = gm::toMat4(rotation);
}
